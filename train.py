...
# Now our output layer is tbhe probability of going up or down. Let's make sure this output is between 0 and 1 by passing it through a sigmoid
...
def train(weights_path = None, render = True):
    ...
    while True:
        ...
        if done: # an episode finished
            ...
            # save model every save_interval times
            if episode_number % save_interval == 0:
                save_weights('weights', weights)
        

def run(weights_path, render = True):
    # load weights
    weights = load_weights(weights_path)
    
    # initialize environment and get first staet
    env = gym.make("Pong-v0")
    observation = env.reset() # This gets us the image
...
def load_weights(path):
    file = open(path,'rb')
    binary = file.read()
    weights = pickle.loads(binary) # This should be a dictionary
    return weights    

def save_weights(path, weights):
    binary = pickle.dumps(weights)
    file = open(path,'wb')
    file.write(binary)
    file.close()
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='starts training or run a trained model')
    ...
    parser.add_argument('--render', type=str, default="True", choices = ["True", "False","true","false"], help = 'chose to render the environment or not')
    
    args = parser.parse_args()
    mode = args.mode
    render = True if str.lower(args.render) == "true" else False
    weights = args.weights
    
    if mode == 'train':
        train(weights,render)
    else:
        if weights is None:
                weights = 'weights'
        run(weights,render)
